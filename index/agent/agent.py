...
class Agent:
    ...
    async def _generate_action(self, input_messages: list[Message]) -> AgentLLMOutput:
        ...
        # Filter out any message that isn't a proper Message object
        cleaned_messages = []
        for msg in input_messages:
            if isinstance(msg, dict) and "type" in msg:
                ...
                cleaned_messages.append(Message(role=role, content=content))
            elif isinstance(msg, Message):
                cleaned_messages.append(msg)
            else:
                continue  # Skip unknown types

        response = await self.llm.call(cleaned_messages)
        try:
